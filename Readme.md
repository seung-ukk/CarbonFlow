...
> **전일 전력 패턴 기반 친환경 가전 사용 타이밍 추천 서비스**  
> 한국전력거래소(KPX) 5분 단위 전력수급 데이터와 Google Gemini AI 에이전트를 결합해, 탄소 배출을 최소화하는 가전 가동 시간을 추천합니다.
...
### 탄소강도 대시보드
- 전일 KPX 데이터를 현재 시각에 맞게 매칭하여 탄소강도 표시
- 탄소강도를 `low / medium / high` 3단계로 시각화
- 재생에너지 비중 및 석탄 발전 비중 추정값 표시
- 15분 주기 자동 갱신

### 12시간 탄소강도 예측
- 전일 패턴 기반으로 향후 12시간의 탄소강도 변화 곡선 제공
- **최적 시간대** (탄소 최저) 및 **주의 시간대** (탄소 최고) 자동 탐색
- 낮 12~14시 태양광 인센티브 가중치 반영

### AI 에이전트 추천
- 가전제품을 선택하면 Gemini 2.5 Flash가 최적 가동 시간 추천 메시지 생성
- 탄소 데이터를 XML 프롬프트로 결합한 컨텍스트 인식 추론
- 3회 리트라이 세이프티 내장
...
| 데이터 처리 | pandas (KPX CSV 시간 슬롯 매칭) |
...
│   │   ├── carbon/
│   │   │   ├── calculator.py    # 탄소강도 연산 엔진 (KPX CSV 시간 매칭)
...
│   │   │   └── kpx_sukub.csv   # 전일 KPX 5분단위 전력수급 데이터 ⬅️ 수동 교체
│   │   └── schemas/
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| `GET` | `/api/carbon/current` | 전일 패턴 기반 현재 탄소강도 조회 |
| `GET` | `/api/carbon/forecast` | 전일 패턴 기반 12시간 탄소강도 예측 |
